...
/**
 * Elevator command that uses a PID Control Loop to bring the Elevator to a
 * target position
 */
public class SetElevatorCommand extends CommandBase {
    ...
    /**
     * returns true if either limit switch is pressed or if encoder position is
     * greater than targetPosition, removing
     * the
    ...
    public boolean isFinished() {
        if (waitForSetpoint) {
            if (Math.abs(ElevatorPolicy.rightEncoderPosition - ElevatorPolicy.setPosition) < ElevatorPolicy.acceptableTolerance) {
                return true;
            } else {
                return false;
            }
