...
public class RobotContainer {
    ...
    //private final LEDSubsystem m_ledSubsystem = new LEDSubsystem();
        ...
        private final CommandJoystick throttleController = new CommandJoystick(
                        OperatorConstants.kThrottleControllerPort);
                ...
                TeleopDrive closedFieldRel = new TeleopDrive(
                                ...
                                () -> (Math.abs(driverController.getRawAxis(1)) > OperatorConstants.LEFT_Y_DEADBAND)
                                                ? driverController.getRawAxis(1)
                                                                * RobotContainer.convertThrottleInput(
                                                                                throttleController.getRawAxis(0))
                                                : 0,
                ...
                new Trigger(
                                ...
                                .whileTrue(new AbsoluteDrive(drivebase,
                                                ...
                                                () -> (Math.abs(driverController
                                                                .getRawAxis(1)) > OperatorConstants.LEFT_Y_DEADBAND)
                                                                                ? driverController.getRawAxis(1)
                                                                                                * RobotContainer.convertThrottleInput(
                                                                                                                throttleController
                                                                                                                                .getRawAxis(0))
                                                                                : 0,
                                                () -> (Math.abs(driverController
                                                                .getRawAxis(0)) > OperatorConstants.LEFT_X_DEADBAND)
                                                                                ? driverController.getRawAxis(0)
                                                                                                * RobotContainer.convertThrottleInput(
                                                                                                                throttleController
                                                                                                                                .getRawAxis(0))
        ...
        //m_ledSubsystem.setDefaultCommand(new NavyLEDCommand(m_ledSubsystem));
                ...
                new JoystickButton(m_operatorController.getHID(), 4).whileTrue(Commands
                                .parallel(new SetElevatorCommand(ElevatorPresets.TRAY_HEIGHT, false),
                ...
                new JoystickButton(m_operatorController.getHID(), 9)
                                .whileTrue(new SetElevatorCommand(ElevatorPresets.HOME, false));
       ...
       // new JoystickButton(m_operatorController.getHID(), 8).onTrue(new ConeLEDCommand(m_ledSubsystem));
        ...
        private static double convertThrottleInput(double input) {
                double output = ((Drivebase.THROTTLE_MAX - Drivebase.THROTTLE_MIN) / 2) * (-input + 1)
                                + Drivebase.THROTTLE_MIN; // input value is negative because the throttle input is
                                                          // reversed by
                                                          // default;
                return output;
        }
